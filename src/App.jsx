  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "#FCFBF8", fontFamily: "'Inter',sans-serif" }}>
      ...
      <div style={{ background: "#fff", border: "2px solid #111", borderRadius: 2, padding: 32, width: 340, boxShadow: "4px 4px 0px #00D2C1" }}>
        ...
        <input type="password" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()} placeholder="Entrez le mot de passe" style={{ width: "100%", padding: "10px 12px", border: `2px solid ${error ? "#D94F4F" : "#E0DDDA"}`, borderRadius: 2, fontSize: 14, fontFamily: "'Inter',sans-serif", background: "#FCFBF8", color: "#111" }} autoFocus />
        {error && <p style={{ color: "#D94F4F", fontSize: 12, marginTop: 8 }}>Mot de passe incorrect</p>}
        <button onClick={handleSubmit} style={{ width: "100%", marginTop: 16, padding: "10px 0", background: "#111", color: "#fff", border: "2px solid #111", borderRadius: 2, fontSize: 12, fontWeight: 600, cursor: "pointer", textTransform: "uppercase", letterSpacing: "0.06em", boxShadow: "3px 3px 0px #00D2C1" }}>Accéder →</button>
      </div>
    </div>
  );
}
...
const DAY_KEYS = ["day_mon", "day_tue", "day_wed", "day_thu", "day_fri"];
const DAY_SHORT = ["L", "M", "Me", "J", "V"];
const DAY_LABELS = ["", "Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi"];

function editorAvailableDayNumbers(w) {
  const days = [];
  if (w.day_mon) days.push(1);
  if (w.day_tue) days.push(2);
  if (w.day_wed) days.push(3);
  if (w.day_thu) days.push(4);
  if (w.day_fri) days.push(5);
  return days;
}

function countDaysInWindow(editorDays, rushDay, deadlineDay) {
  const start = rushDay > 0 ? rushDay : 1;
  const end = deadlineDay > 0 ? deadlineDay : 5;
  return editorDays.filter(d => d >= start && d <= end).length;
}
...
function priorityScore(d) {
  ...
  if (d.desired_deadline_day > 0 && d.desired_deadline_day <= 2) s += 3;
  return s;
}
...
function computePlanning(demands, editors, affinityMap) {
  ...
  const pool = editors
    ...
    .map(e => ({ ...e, capacity: e.totalDays * 5, used: 0 }));
  ...
  for (const client of scored) {
    ...
    const tryAssign = (list) => {
      for (const ed of list) {
        if (rem <= 0) break;
        const globalRemaining = ed.capacity - ed.used;
        if (globalRemaining <= 0) continue;

        // Compute effective capacity for this client based on work window
        const daysInWindow = countDaysInWindow(ed.availableDays, client.rush_day || 0, client.desired_deadline_day || 0);
        const windowCapacity = daysInWindow * 5;
        if (windowCapacity <= 0) continue;

        const can = Math.min(rem, windowCapacity, globalRemaining);
        if (can > 0) {
          assignments.push({
            ...
            desired_deadline_day: client.desired_deadline_day,
...
export default function App() {
  ...
  useEffect(() => { if (!loading) loadWeek(weekStart); }, [weekStart]);
  ...
  const affinityMap = useMemo(() => {
    const m = {};
    affinities.forEach(a => { if (!m[a.client_id]) m[a.client_id] = []; m[a.client_id].push(a.editor_id); });
    return m;
  }, [affinities]);
  ...
  const planning = useMemo(() => {
    const demands = clients.map(c => {
      ...
      return { client_id: c.id, client_name: c.name, pack: c.pack, concepts_requested: w.concepts_requested || 0, at_risk: w.at_risk || false, quality_required: w.quality_required || false, behind_schedule: w.behind_schedule || false, has_deadline: w.has_deadline || false, deadline_date: w.deadline_date || "", rush_day: w.rush_day || 0, desired_deadline_day: w.desired_deadline_day || 0 };
    });
    const eds = editors.map(e => {
      const w = weekEditors.find(we => we.editor_id === e.id);
      const availDays = w ? editorAvailableDayNumbers(w) : [1, 2, 3, 4, 5];
      return { id: e.id, name: e.name, level: e.level, is_freelance: e.is_freelance, totalDays: availDays.length, availableDays: availDays };
    });
  ...
  const totalCap = useMemo(() => editors.reduce((s, e) => {
    const w = weekEditors.find(we => we.editor_id === e.id);
    const days = w ? editorAvailableDayNumbers(w).length : 5;
    return s + days * 5;
  ...
  // ── CRUD ──
  const addEditor = async () => { const { data } = await supabase.from("editors").insert({ name: "", level: "confirmed", is_freelance: false }).select().single(); if (data) setEditors(p => [...p, data]); flash(); };
  const updateEditor = async (id, field, value) => { await supabase.from("editors").update({ [field]: value }).eq("id", id); setEditors(p => p.map(e => e.id === id ? { ...e, [field]: value } : e)); flash(); };
  const deleteEditor = async (id) => { await supabase.from("editors").delete().eq("id", id); setEditors(p => p.filter(e => e.id !== id)); flash(); };
  const addClient = async () => { const { data } = await supabase.from("clients").insert({ name: "", pack: 1, strategist: "" }).select().single(); if (data) setClients(p => [...p, data]); flash(); };
  const updateClient = async (id, field, value) => { await supabase.from("clients").update({ [field]: value }).eq("id", id); setClients(p => p.map(c => c.id === id ? { ...c, [field]: value } : c)); flash(); };
  const deleteClient = async (id) => { await supabase.from("clients").delete().eq("id", id); setClients(p => p.filter(c => c.id !== id)); flash(); };

  const toggleAffinity = async (clientId, editorId) => {
    const exists = affinities.find(a => a.client_id === clientId && a.editor_id === editorId);
    if (exists) { await supabase.from("affinities").delete().eq("id", exists.id); setAffinities(p => p.filter(a => a.id !== exists.id)); }
    else { const { data } = await supabase.from("affinities").insert({ client_id: clientId, editor_id: editorId }).select().single(); if (data) setAffinities(p => [...p, data]); }
    flash();
  };

  const updateWeekEditor = async (editorId, updates) => {
    const existing = weekEditors.find(w => w.editor_id === editorId);
    if (existing) {
      await supabase.from("weekly_editor_availability").update(updates).eq("id", existing.id);
      setWeekEditors(p => p.map(w => w.id === existing.id ? { ...w, ...updates } : w));
    } else {
      const row = { week_start: weekStart, editor_id: editorId, days_available: 5, day_mon: true, day_tue: true, day_wed: true, day_thu: true, day_fri: true, ...updates };
      const { data } = await supabase.from("weekly_editor_availability").insert(row).select().single();
  ...
  const updateWeekClient = async (clientId, field, value) => {
    ...
    if (existing) {
      ...
      const row = { week_start: weekStart, client_id: clientId, concepts_requested: 0, at_risk: false, quality_required: false, behind_schedule: false, has_deadline: false, deadline_date: null, rush_day: 0, desired_deadline_day: 0, [field]: value };
  ...
  return (
    ...
    <div style={S.root}>
      ...
      <div style={S.tabBar}>
        {[{ key: "ref", label: "Référentiel" }, { key: "affinities", label: "Affinités" }, { key: "week", label: "Semaine" }, { key: "planning", label: "Planning" }].map(t => (
...
function RefTab({ editors, clients, addEditor, updateEditor, deleteEditor, addClient, updateClient, deleteClient }) {
  ...
  const debounce = (fn, id, field, value, delay = 500) => {
    const key = `${id}-${field}`;
  ...
  return (
    <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
      <div style={{ flex: 1, minWidth: 340 }}>
        <div style={S.sectionHeader}><h2 style={S.sectionTitle}>CLIENTS</h2><button onClick={addClient} style={S.addBtn}>+ Client</button></div>
        ...
        {localClients.map(c => (
          <div key={c.id} style={S.row}>
            <div style={{ flex: 2 }}><input style={S.input} placeholder="Nom du client" value={c.name} onChange={e => { const v = e.target.value; setLocalClients(p => p.map(x => x.id === c.id ? { ...x, name: v } : x)); debounce(updateClient, c.id, "name", v); }} /></div>
            <div style={{ flex: 1, display: "flex", gap: 3 }}>
              {[1, 2, 3].map(s => (<button key={s} onClick={() => updateClient(c.id, "pack", s)} style={{ ...S.sizeBtnSm, ...(c.pack === s ? { background: C.black, color: "#fff", borderColor: C.black } : {}) }}>P{s}</button>))}
            </div>
            <div style={{ flex: 1.5 }}><input style={S.input} placeholder="CS attitré" value={c.strategist} onChange={e => { const v = e.target.value; setLocalClients(p => p.map(x => x.id === c.id ? { ...x, strategist: v } : x)); debounce(updateClient, c.id, "strategist", v); }} /></div>
            <div style={{ flex: 0.3, display: "flex", justifyContent: "center" }}><button onClick={() => deleteClient(c.id)} style={S.delBtn}>×</button></div>
          </div>
        ))}
        {clients.length === 0 && <p style={S.empty}>Ajoute tes clients pour commencer.</p>}
      </div>
      <div style={{ flex: 1, minWidth: 340 }}>
        <div style={S.sectionHeader}><h2 style={S.sectionTitle}>ÉQUIPE</h2><button onClick={addEditor} style={S.addBtn}>+ Monteur</button></div>
        <div style={S.tableHeader}>
          <span style={{ ...S.th, flex: 2 }}>Nom</span>
        ...
        {localEditors.map(e => (
          <div key={e.id} style={S.row}>
            <div style={{ flex: 2 }}><input style={S.input} placeholder="Nom" value={e.name} onChange={ev => { const v = ev.target.value; setLocalEditors(p => p.map(x => x.id === e.id ? { ...x, name: v } : x)); debounce(updateEditor, e.id, "name", v); }} /></div>
            <div style={{ flex: 1 }}><select style={S.select} value={e.level} onChange={ev => updateEditor(e.id, "level", ev.target.value)}><option value="senior">Senior</option><option value="confirmed">Confirmé</option><option value="junior">Junior</option></select></div>
            <div style={{ flex: 0.7, display: "flex", justifyContent: "center" }}><button onClick={() => updateEditor(e.id, "is_freelance", !e.is_freelance)} style={{ ...S.toggleBtn, background: e.is_freelance ? C.accent + "15" : "transparent", borderColor: e.is_freelance ? C.accent : C.border, color: e.is_freelance ? C.accentDark : C.textLight }}>{e.is_freelance ? "Oui" : "Non"}</button></div>
            <div style={{ flex: 0.3, display: "flex", justifyContent: "center" }}><button onClick={() => deleteEditor(e.id)} style={S.delBtn}>×</button></div>
          </div>
        ))}
...
function AffinitiesTab({ editors, clients, affinities, toggleAffinity }) {
  ...
  if (!activeCl.length || !active.length) return <div><h2 style={S.sectionTitle}>AFFINITÉS MONTEUR × CLIENT</h2><p style={{ ...S.empty, marginTop: 40 }}>Ajoute au moins un client et un monteur dans le Référentiel.</p></div>;

  return (
    <div>
      <div style={S.sectionHeader}><div><h2 style={S.sectionTitle}>AFFINITÉS MONTEUR × CLIENT</h2><p style={S.sectionDesc}>Coche les monteurs habitués à chaque client. Contrainte forte : ils seront assignés en priorité.</p></div></div>
      <div style={{ overflowX: "auto" }}>
        <table style={{ borderCollapse: "collapse", width: "100%" }}>
          <thead><tr>
            <th style={{ ...S.matrixTh, textAlign: "left", minWidth: 160 }}>Client</th>
            {active.map(e => (<th key={e.id} style={S.matrixTh}><div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}><span style={{ fontSize: 12, fontWeight: 600 }}>{e.name}</span><span style={{ fontSize: 9, color: LEVEL_CLR[e.level].text, background: LEVEL_CLR[e.level].bg, padding: "1px 6px", borderRadius: 2, fontWeight: 700, textTransform: "uppercase" }}>{LEVELS[e.level]}</span></div></th>))}
          </tr></thead>
          <tbody>
            {activeCl.map(c => (
              <tr key={c.id}>
                <td style={S.matrixTd}><span style={{ fontWeight: 600, fontSize: 13 }}>{c.name}</span><span style={{ fontSize: 10, color: C.textLight, marginLeft: 6 }}>{PACK_LABELS[c.pack]}</span></td>
                {active.map(e => {
                  const on = affinities.some(a => a.client_id === c.id && a.editor_id === e.id);
                  return (<td key={e.id} style={{ ...S.matrixTd, textAlign: "center" }}><button onClick={() => toggleAffinity(c.id, e.id)} style={{ width: 32, height: 32, borderRadius: 2, border: `2px solid ${on ? C.accent : C.border}`, background: on ? C.accent : "transparent", cursor: "pointer", display: "inline-flex", alignItems: "center", justifyContent: "center", color: on ? "#fff" : C.border, fontSize: 16, fontWeight: 700, boxShadow: on ? `2px 2px 0px ${C.black}` : "none", transition: "all .15s" }}>{on ? "✓" : ""}</button></td>);
                })}
              </tr>
            ))}
          </tbody>
...
function PlanningTab({ planning, totalCap, totalDem }) {
  ...
  return (
    <div>
      <div style={S.sectionHeader}><div><h2 style={S.sectionTitle}>PLANNING DE LA SEMAINE</h2><p style={S.sectionDesc}>Répartition automatique. Affinités en priorité. Fenêtre de travail = entre arrivée rushs et livraison espérée, selon les jours de dispo du monteur.</p></div></div>

      {totalDem > totalCap && (
        <div style={S.alertBox}><span style={{ fontSize: 18 }}>⚠</span><div><strong>Surcharge</strong> — {totalDem} concepts demandés vs {totalCap} de capacité. {totalDem - totalCap} concepts reportés.</div></div>
      )}
            ...
            <div style={S.planLeft}>
              ...
              <div>
                ...
                <div style={S.planMeta}>
                  ...
                  {data.flags.desired_deadline_day > 0 && <span style={{ color: C.purple, marginLeft: 8 }}>Livraison : {DAY_LABELS[data.flags.desired_deadline_day]}</span>}
                </div>
              </div>
            </div>
                ...
                <div key={e.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 2, padding: "12px 16px" }}>
                  <div style={S.barRow}>
                    ...
                    <div style={{ display: "flex", gap: 2, width: 90 }}>
                      {[1,2,3,4,5].map(d => (
                        <div key={d} style={{ width: 14, height: 14, borderRadius: 2, background: e.availableDays.includes(d) ? C.accent : C.border, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 7, fontWeight: 700, color: e.availableDays.includes(d) ? "#fff" : C.textLight }}>{DAY_SHORT[d-1]}</div>
                      ))}
                    </div>
...
const S = {
  ...
  dispoCard: { padding: "12px 14px", background: C.card, border: `1px solid ${C.border}`, borderRadius: 2 },
